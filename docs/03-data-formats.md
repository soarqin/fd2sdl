...
原版的增量刷新路径会把 312×192 内区复制到 VGA `(4,4)`，但场景初始化先填满 320×200，边缘沿用完整地图画面，并非固定黑框。SDL 每帧从零重建画面，因此应按地图像素边界钳制镜头并填满 320×200；不能在增量复制后等价地清除四边 4 px。
...
| `-17` (`0xffef`) | 顶部对话框，下一 token 为 DATO 立绘编号 |
| `-18` (`0xffee`) | 底部对话框，下一 token 为 DATO 立绘编号 |
...
`dialog_box_open @0x13cf4` 先将 FDOTHER.DAT[5] tile 0 的 24×24 空心框从说话角色所在格移动到对话框原点，再按 `4×2 → 8×3 → 12×4 → 16×5 → 19×5` 个内部块绘制边框，形成伪缩放弹出效果。对话结束或切换说话角色时执行反向收起。
...
移动阶段使用 actor record offset `0x04` 的 `1..6` 相位，每帧沿方向偏移 4 px，第 6 帧跨满 24 px。`field_animation_phase_update @0x100c5` 约每 275 ms 推进 idle 相位；`map_actor_blit_24x24 @0xb168` 将 phase 3 映射回 phase 1，可见序列为 `0,1,2,1`。
