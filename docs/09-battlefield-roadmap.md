...
当前实现已形成第一关可交互战场骨架，包含移动、回合、原版图形指令菜单和无演出的确定性普通攻击；正式敌方 AI、法术、道具及关卡结束流程仍未完成。
...
- 法术、道具、经验和升级。
- 正式敌方 AI、通用 cell action handler。
- 胜负判断、增援和关卡推进。
- FIGANI 战斗演出。
- 完整存档恢复和写回。
...
- [x] Ghidra 标注、`docs/function-names.md` 与源码逆向注释已同步。
...
→ side 1 自动待机 → side 0 自动待机 → 回合数加一 → 玩家阶段
```
...
- [x] `DS:0x1b91` 已确认为战场 action 表。object1 fixup 到统一 code0 使用 `target-0x28b8+0x27acc`；DOSBox 运行时 `action 0 -> 0x190531` 与 `code0 0x49745` 机器码一致。
- [x] 回调 ABI 已确认：cdecl 单一 32 位 actor index 参数，回合事件传 0，调用者清理 4 字节栈，返回值不使用。
- [x] stage 0/31 action `0..3` 入口定位到 corrected dual `0x59745/0x5981f/0x59887/0x598e1`（code0 `0x49745/0x4981f/0x49887/0x498e1`）；action 1/2 分别加入 group 4/5，其余镜头、移动和对话调用也已静态确认。
...
当前进度：M7.0 的单位 AI 字段与纯查询层已完成，尚未替换 side 1/0 的自动待机提交。
...
- [ ] 继续拆分 mode 0 在移动前依次调用的 `0x3a104/0x39335` 攻击／技能判定，以及 `field_ai_move_toward_cell` 的长路径 fallback。当前仅确认 `0x3a104` 的机器码调用结构：先调用 `0x3944b/0x3ab9e/0x3a892` 构造评分全局 `DS:0x3c23/0x3c33/0x3c4f`（阈值 6），比较后按分支调用 `field_physical_exchange @0x3a6a2`、`0x3a525` 或 `0x3a269` 真正结算攻击；三个评分 helper 的语义尚未确认，不得据此命名或实现。该分支会提交战斗状态，属独立里程碑步，须先完成评分 helper 的 clean 反汇编与 DOSBox 交叉验证。
- [ ] 建立 AI plan/commit 边界后，替换 side 1/0 自动待机；不能直接复用限定 side 2 → side 0 的玩家 `begin_attack`。
- [ ] 复用已确认的路径和战斗计算，实现目标选择、移动、攻击或待机。
- [ ] 增援组激活、胜利、失败和 stage 结束。
