...
|---|---:|---|---|
| AH | `0x01` | Esc；`0x53` 规范化后的取消码 | 通用二选一框取消；战场控制器分支 |
| AH | `0x1c` | Enter；`0x52` 或 `0xe0` 规范化后的确认码 | 战场确认、通用二选一框确认 |
...
- SDL 的方向键与数字小键盘方向键映射到同一原版方向动作；Return、Space、数字小键盘 0/Enter 映射到确认。通用二选一框将 Escape、数字小键盘小数点／Delete 映射到取消；战场则把它们与 Z、数字小键盘 5 保留为同一焦点更新动作，未验证前不猜测为取消。
- 标题、通用菜单、战场、调试预览各自定义允许动作；不把一个上下文的 Esc 退出语义扩散到原版菜单。
