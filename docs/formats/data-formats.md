...
`new_game_opening_play @0x3231b` 的完整开场依次使用三个 stage：
...
stage 31 的前五个单位模板在 offset `0x15` 分属 group `1,1,3,3,5`。`new_game_opening_play @0x3231b` 在初始镜头 `(5,42)` 只显示 actor 0/1；fragment 2 后加入 group 3 的 actor 2/3，再将镜头卷到 `(4,41)`。actor 2（unit 75）与 group 5 的 actor 4（unit 9）共用 placement `(5,44)`；原流程先隐藏 actor 2，之后才加入 group 5，因此两者不会同时绘制。一次性加载前五名 actor 会造成初始画面提前出现左侧两人，并让倒地与站立 sprite 重叠。
...
`DAT_000027d8` 指针表位于 object3；LE fixup 的 `target_offset` 直接映射为 object3 内偏移，再加 object3 relbase。按此映射可完整解码 script `0..10` 与开场脚本 `0x5a..0x69`；其中 script `3/4/6/7/8` 已用于 stage 0 回合事件，`0/1/2/5/0x5a..0x69` 与 `new_game_opening_play @0x3231b` 的直接调用顺序一致。
...
| `FDOTHER.DAT[31]` | `DAT_00003eec` 固定加载的 13 项 unsigned 8-bit mono PCM bank；SFX 5/6=详情页开合、8=战场指令菜单展开／收起 |
...
| [31] | UI／战场数字音效 bank | 13 | `sfx_play(DAT_00003eec,index,loop_count)` 按 offset 直接提交样本字节与长度；详情页和战场菜单使用该 bank |
