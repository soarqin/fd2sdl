...
| `FDOTHER.DAT[31]` | `DAT_00003eec` 固定加载的 13 项 unsigned 8-bit mono PCM bank；SFX 1=标题确认、2=标题选择移动、5/6=详情页开合、8=战场指令菜单展开／收起 |
| `FDOTHER.DAT[77]` | 4 项 secondary unsigned 8-bit mono PCM bank；标题 action 入口播放 SFX 3 |
...
| [31] | UI／战场数字音效 bank | 13 | `sfx_play(DAT_00003eec,index,loop_count)` 按 offset 直接提交样本字节与长度；标题移动／确认分别使用 SFX 2/1 |
| [77] | secondary 数字音效 bank | 4 | 标题 action 入口通过 secondary sample handle 播放 SFX 3 |
