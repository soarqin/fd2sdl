...
| 0x45834 | FUN_0000dd68 | input_check | 检查输入(按键) | 反编译：事件泵调用 |
...
| 0x43edb | FUN_0001c40f | field_physical_attack_resolve | 结算一次普通物理攻击的命中、暴击、伤害和目标 HP | 反编译确认 record `0x48/0x4a/0x4c/0x4e` 分别用于攻击、防御、命中、回避；核心判定为 `rand()%100 < accuracy-evasion`，暴击时防御减半，基础伤害为 `(attack-defense)*9/10`，再加 `rand()%(base/9)` 浮动并将 HP 钳制到 0 |
