...
| 0x35834 | FUN_00035834 | input_check | 非消费式检查 BIOS 键盘缓冲是否有待取按键 | 重建后的 raw `0x35843..0x35859` 比较 BDA `0x041a/0x041c` 的 head/tail；`animation_play @0x45635` 在 `0x4577c` 调用。旧登记 `0x45834` 落在战场初始化清零指令中，不是函数入口；详见 `docs/12-input-system.md`。 |
...
| 0x7370a | FUN_0004bc3e | field_path_find | 使用同一成本和占格规则搜索目标并输出方向路径 | corrected code0 `0x6370a` 接收起终点、预算、路径 buffer 与模式参数；内部按 attr[1] 索引 profile 成本 |
...
| 0x38cb3 | Ghidra 未识别入口 | field_ai_unit_execute | 按单位 AI behavior 执行一名 side 0/1 actor | corrected code0 `0x28cb3` 为 `__chkstk` 入口；先拒绝 flags `&0x05`，再取 record `+0x34 & 0x0f` 分派 behavior `0..5、7..11`，并读取 `+0x35/+0x36/+0x3d` 参数。无论动作分支，正常尾部都会查询 cell event 并调用 `field_actor_mark_acted`。 |
| 0x38cbd | Ghidra 未识别边界 | field_ai_unit_execute_core | `field_ai_unit_execute` 的 `__chkstk` 后主体 | mode 0 的顺序为 `0x3a104 → 0x39335 → field_ai_move_toward_nearest`，前一项成功即停止；其他 mode 的完整动作语义仍待逐项确认。 |
...
| 0x39d8c | Ghidra 未识别入口 | field_ai_move_toward_cell | 计算向指定坐标移动的可达目的地和路径并播放移动 | corrected code0 `0x29d8c` 构造 ZOC、长路径 fallback、可达表与友方不可停留过滤；按行优先枚举可停留格，先最小化曼哈顿距离，平价时最小化机器码实际分数 `abs(dx-abs(dy))`，最后重建路径并调用 `field_actor_path_play`。 |
| 0x39d96 | Ghidra 未识别边界 | field_ai_move_toward_cell_core | `field_ai_move_toward_cell` 的 `__chkstk` 后主体 | SDL `field_ai` 当前只复现最终目的地稳定排序；长路径 fallback 与动作提交尚未接入。 |
...
| 0x59745 | LAB_00031c79 | field_stage0_31_turn_action0 | stage 0/31 第 3 回合 side 1 剧情 action 0 | corrected dual `0x59745` / code0 `0x49745`；`DS:0x1b91[0]` fixup、FD2.EXE file `0x5a545` 和 DOSBox 运行时入口三重确认；直接加入 group 3/7，镜头移至 `(5,8)`，执行 script 7/8，播放 fragment 11/3 |
| 0x5981f | LAB_00031d53 | field_stage0_31_turn_action1 | stage 0/31 第 4 回合 side 0 剧情 action 1 | corrected dual `0x5981f` / code0 `0x4981f`；镜头移至 `(11,16)`，以 LMI1 特效加入 group 4，执行 script 3，播放 fragment 4 |
| 0x59887 | LAB_00031dbb | field_stage0_31_turn_action2 | stage 0/31 第 5 回合 side 0 剧情 action 2 | corrected dual `0x59887` / code0 `0x49887`；镜头移至 `(0,16)`，以 LMI1 特效加入 group 5，执行 script 4，播放 fragment 5 |
| 0x598e1 | LAB_00031e15 | field_stage0_31_turn_action3 | stage 0/31 第 6 回合 side 1 剧情 action 3 | corrected dual `0x598e1` / code0 `0x498e1`；镜头移至 `(11,11)`，直接加入 group 6，执行 script 6，播放 fragment 6 |
| 0x3a6a2 | Ghidra 未识别边界 | field_physical_exchange | 执行一次进攻序列，并在目标存活且满足条件时交换双方执行反击序列 | corrected code0 `0x2a7b5..0x2a7bd` 以 `attacker,target,frames` 调用 `0x43a6a`；返回 HP 非 0 且 `field_counterattack_is_available` 返回 1 时，`0x2a81a..0x2a822` 交换 attacker/target 再调用。反击不写行动标志。 |
...
| 0x42a29 | Ghidra 未识别边界 | field_side1_phase_execute_core | `field_side1_phase_execute` 的 `__chkstk` 后主体 | actor 顺序严格为索引递增；全局退出标志置位时提前结束。 |
| 0x42ace | Ghidra 未识别入口 | field_side0_phase_execute | 两阶段执行 side 0 单位阶段 | corrected code0 `0x32ace` 首轮对每个可行动 side 0 actor 调用 `0x3ab9e/0x3a892` 并按计数条件决定是否执行 AI；第二轮再对同一筛选条件调用 `field_ai_unit_execute(actor,0)`。两个前置 helper 的评分语义仍待确认。 |
| 0x42ad8 | Ghidra 未识别边界 | field_side0_phase_execute_core | `field_side0_phase_execute` 的 `__chkstk` 后主体 | 两轮均在每个 actor 后执行 cell action 与 stage handler，并检查全局退出标志。 |
