...
| 0x3dfa0 | FUN_0003dfa0 | field_player_command_execute | 显示并执行玩家单位移动后的指令菜单 | `0x3dfaa` 主体先构造禁用项，再按菜单结果分派。普通攻击分支查已装备武器、按 item record 范围构造目标集合、进入格选择并取得目标 actor；取消时返回 `-1` 且不标记 acted。法术、道具等其他分支仍待细分。 |
| 0x3dfaa | FUN_0003dfaa | field_player_command_execute_core | `field_player_command_execute` 的 Watcom `__chkstk` 后主体 | corrected 机器码入口 `0x3dfa0` 为栈检查前缀，主体从 `0x3dfaa` 开始。 |
...
| 0x3f51f | FUN_00017a53 | field_turn_cycle_run | 玩家阶段结束后依次处理 side 1、side 0，递增回合后进入 side 2 | 反编译调用顺序为 `field_turn_event_check(1)`、side 1 actor 处理、清行动标志、`field_turn_event_check(0)`、side 0 actor 处理、`DAT_00003bef++`、清行动标志、`field_turn_event_check(2)`；机器码 `0x3f716..0x3f731` 比较 primary/alternate 并按需淡出，`0x3f78e` 在 phase 0 事件后读取 `DS:0x1e81[stage]` 启动 alternate，`0x3f824` 在新回合 phase 2 前恢复 `DS:0x1e63[stage]` primary |
