...
当前进度：M7.0～M7.2 查询／计划层及 M7.3a 显式物理提交与受限 mode0 物理自动执行已完成；magic/item、mode-2／mode-1 精确移动和 side 1/0 完整原版 phase hook 仍未完成。
...
- [ ] M7.3b：继续逆向并移植可确认的 `0x3a525` 法术 handler、`0x3a269→0x45e83` 物品效果／条件消耗，以及 `0x39335` mode-2 witness、`0x39d8c` mode-1 长路径；当前对缺少权威数据的分支显式拒绝，不降级为物理或待机。
- [ ] 完整 commit 边界确认后替换 side 1/0 自动待机；不能直接复用限定 side 2 → side 0 的玩家 `begin_attack`。
