...
magic 和 item 的身份、原版入口及禁用条件已经确认；执行状态机尚未实现，因此当前显示 disabled 帧。下一开发阶段为 M7 敌方 phase AI。
...
- [ ] 完成 `0x3a525` 法术 handler、`0x3a269→0x45e83` 物品效果／条件消耗，以及 `0x39335` 第一 fallback；确认 acted/phase 后再把 mode 0 plan/commit 接入 side 1/0 phase
- [ ] FIGANI.DAT 战斗演出
- [ ] 经验/升级
