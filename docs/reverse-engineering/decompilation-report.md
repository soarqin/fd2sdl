...
| fixup_size | +0x30 | 0xe1ba | fixup section 大小 |
| loader_size | +0x38 | 0xe325 | loader section 大小 |
| data_pages_off | +0x80 | 0x10e00 | 相对 embedded MZ module `@0x25214`；page 1 file offset 为 `0x36014` |
...
- 页面映射表（object page map）为恒等映射：逻辑页 i → 物理页 i。数据页相对 embedded MZ module `@0x25214` 的偏移为 `0x10e00`，因此 page 1 的文件位置为 `0x36014`。
...
- FD2 共 7959 条记录、7948 个唯一 source；11 条重复记录指向同一 source 和 target，没有冲突。
- 记录只有两种：`07 00 <src:i16> <obj:u8> <off:u16>` 共 7067 条，`07 10 <src:i16> <obj:u8> <off:u32>` 共 892 条。
- `src` 是相对该 fixup page 的 signed i16，允许指向上一页末尾；`target_offset` 直接是目标 object 内偏移。
- 完整记录由主 parser 和独立 decoder 逐条比较；每个 source 的文件原值也必须等于记录中的 target offset。
...
Ghidra 可输出可读 C 伪代码；当前全量生成结果见 `docs/generated/ghidra-decomp-all.c`。
r2 可输出带交叉引用的汇编。
...
- 重写不复刻 AIL 硬件驱动，但先以 SDL3 `SDL_AudioStream` 建立统一框架，再分别接入 PCM 音效与 XMIDI/OPL source。见 `docs/plans/audio.md`
...
## 8. 启动画面流程（详见 docs/reverse-engineering/boot-flow.md）
...
Ghidra/r2 不自动应用 LE fixup。旧流程的问题不是「fixup 不能写入代码页」，而是 page base 和 target 换算错误，导致把 relocation 写到完全错误的指令位置。

当前流程：
- `tools/rebuild_fd2_analysis.py` 分开生成 raw-unrelocated、loader-relocated 和 relocation manifest；
- `tools/validate_le_fixups.py` 使用独立 decoder 逐条比较，并验证全部 source prevalue、重复记录和非 source 字节不变；
- `docs/generated/le-fixups.txt` 保存摘要，完整索引在 `tools/fd2_le_fixups.txt`；
- Ghidra clean 输入仍使用 raw object1；relocated 镜像只用于验证和解释运行时绝对引用；
