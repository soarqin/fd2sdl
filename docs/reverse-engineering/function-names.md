...
| 0x42ad8 | Ghidra 未识别边界 | field_side0_phase_execute_core | `field_side0_phase_execute` 的 `__chkstk` 后主体 | 两轮均在每个 actor 后执行 cell action 与 stage handler，并检查全局退出标志 `DS:0x3ecc`；`0x205be` 会将其置为 2，普通退出处理可置为 1。该标志不是通用 battle result。`DS:0x1b19[0]` 的 stage 0 handler 正确 dual 为 `0x205b4`，其结束门检查 side 0 actor 表和 actor 0 主角隐藏状态。 |
