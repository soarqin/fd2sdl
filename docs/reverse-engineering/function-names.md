...
| 0x1f894 | Ghidra 未识别边界 | title_action_menu | 标题 action 菜单；无活动快照时仅显示两项，有活动快照时插入 active snapshot load | 当前权威 code0 `0xf894`；先以 `FDTXT`/`FD2.SAV` 读取活动快照并检查 `meta[2] == 0`，再调用通用 six-entry selector `0x17d33`；该链不调用 `new_game_opening_play @0x2231b`，四槽 hand_load 仍属于 code0 `0x19300` |
