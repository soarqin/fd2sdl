...
| 0x44ab2 | FUN_0001cfe6 | boot_intro_title | 片头+标题主体 | r2 原始字节：从 `push ebx` @0x1cfe6 开始；corrected code0 `0x15db1` 在进入 `title_action_dispatch` 前调用 `music_track_play(18,0)`，因此片头与标题共用循环 track 18；DOSBox 对照 |
| 0x1f894 | Ghidra 未识别边界 | title_action_menu | 标题 action 菜单；无活动快照时仅显示两项，有活动快照时插入 active snapshot load | 当前权威 code0 `0xf894`；`0xf8d6..0xf8e6` 将 FDOTHER[77] 保存到 local `+0x54`，后续标题音效均传入该 local：`0xfbb7..0xfbd6` 在 object2 `DS:0x204e` 的 14 个有效滚动阈值以 primary wrapper 重启 SFX 0；`0xfbd9..0xfc37` 在全部 14 个有效触发帧切换到 FDOTHER[102] 闪电调色板，并在 `local_4c` 逐帧计至 11 后恢复 FDOTHER[101]（11 个滚动帧）；`0xfd1c` 以 secondary wrapper 播放 SFX 3，`0xfe74/0xfe94` 以 primary wrapper 播放 SFX 2，`0xfed3` 播放 SFX 1。菜单先以 `FDTXT`/`FD2.SAV` 读取活动快照并检查 `meta[2] == 0`，再调用通用 six-entry selector `0x17d33`；确认后 `0xfef0..0xff30` 将选中项 normal/highlight 各显示 80 ms，交替 4 轮。该链不调用 `new_game_opening_play @0x2231b`，四槽 hand_load 仍属于 code0 `0x19300` |
| 0x25ebb | Ghidra 未识别边界 | title_action_dispatch | 标题 action 上层循环与返回分派 | 当前权威 code0 `0x15ebb`；调用 `title_action_menu @0xf894`，返回 0 时进入新游戏／初始化，返回 1 时读取活动快照并恢复，其他返回值退出或回到宿主；具体可见标题标签仍待 DOSBox 确认 |
| 0x46f54 | FUN_0000f488 | dac_apply_darkness | 应用当前调色板到 VGA DAC，第三参数为暗度(0x40=黑,0=全亮) | 调用点：FUN_0001cc6d/FUN_0001cfca 亮暗循环；SDL 对照修正 |
| 0x45635 | FUN_0001db69 | animation_play | ANI.DAT/AFM 脚本动画播放器(anim_idx, frame_delay, check_input) | corrected code0 `0x10421`：打开 ANI.DAT、读取 `0xad` 记录头、逐帧调用 `anim_exec_bytecode`；当 `anim_idx == 1` 时，`0x1043f..0x10457` 加载 FDOTHER[78]，`0x1053b..0x10552` 在 frame 0 解码后、首个 delay 前以 primary sample handle 播放 SFX 0，`0x10586..0x1059a` 在正常结束或跳过后停止。SDL 已实现 opcode 1/2/4/5/6/7/8/9 |
...
| code0 0x5f84（dual 0x15f84） | 旧 Ghidra 错分为 FUN_000136cc | text_dialog_render_tokens | 渲染 FDTXT u16 token：处理换行、分页、动态嵌入、立绘控制码和 16×16 字模 | 当前权威 code0 直接反汇编确认：-4/-5 递归嵌入动态文本，-6 绘制十进制数值，-0x13/-0x14 通过 `DAT_00003a45 + idx*0x50 + 7` 取 DATO 立绘；旧生成文件的 `0x3b198` 地址与当前 code0 不一致，不再作为权威地址 |
...
| code0 0x64e8（dual 0x164e8） | Ghidra 未识别边界 | text_dialog_glyph_step | 每个可见字形的音效、口型动画更新与逐字延时 | 当前权威 code0 `0x653c..0x6550` 直接压入 `loop_count=1`、`sample=2`、`DAT_00003eec`，调用 primary SFX wrapper `0x15a96`，随后调用 `bios_tick_delay(1)`；普通字形路径 `0x64a2` 与动态数字路径 `0x6139` 均调用此 helper |
| code0 0x7aa9（dual 0x17aa9） | 旧 Ghidra 错记为 FUN_000151f1 @0x3ccbd | bios_tick_delay | 按 BIOS 计时器等待指定 tick | 当前权威 code0 读取物理地址 `0x046c` 的低 16 位计时器；1 tick 约 54.9 ms；`text_dialog_glyph_step` 每字调用 `bios_tick_delay(1)` |
...
| 0x386f8 | FUN_000386f8 | field_all_actor_directions_reset | 将完整 actor 表的方向字段归零并延迟 20 ms | corrected dual `0x134e4` 遍历 `DAT_00003a45` 的全部 `DAT_00003beb` 条记录，写 `record[0x03]=0`，随后调用 `delay_ms(0x14)`；`field_ai_unit_execute` common tail 在 cell lookup、mark acted 之后调用。 |
| 0x3874a | FUN_00010c7e | field_all_actors_clear_acted | 清除全部 actor 的已行动标志 | 反编译遍历 `DAT_00003a45` 并执行 `record[0x05] &= 0x7f`；阶段切换前调用 |
| 0x117e7 | Ghidra 未识别边界 | field_controller_input | 处理战场 browse 状态的一次按键 | 当前权威 code0 `0x17e7`，dual `0x117e7`。Enter/Space 先调用 code0 `0x2c0d` 的可见 actor 查询；焦点无可见 actor 时唯一直接调用 `field_empty_focus_menu_execute @0x16f55`。Esc/Delete、Z 和 keypad 5 共用玩家单位焦点轮换分支；F1/Page Up 进入独立缩放战术图页面，不是系统菜单。 |
| 0x16f55 | Ghidra 未识别边界 | field_empty_focus_menu_execute | 焦点无可见 actor 时展开四项 field-level 菜单 | 当前权威 code0 `0x6f55`，唯一直接调用点为 code0 `0x18c1`。command IDs 来自 object 2 `DS:0x1e9f={7,5,6,4}`；selector 0 调用 `field_secondary_menu_execute @0x19df7`，selector 1 使用 FDTXT `0x1a1/0x1a2` 确认行军，selector 2 调用 `field_options_menu_execute @0x1728c`，selector 3 使用 FDTXT `0x1a3/0x1a4` 确认结束回合。command ID 不等于用户可见标签；标签仍待 DOSBox 截图。 |
| 0x1728c | Ghidra 未识别边界 | field_options_menu_execute | 循环显示四项战场设置开关 | 当前权威 code0 `0x728c`，主体从 code0 `0x7296`（dual `0x17296`）开始；object 2 `DS:0x1eaf={18,20,22,24}` 提供基础 command IDs，运行时生成 `18/19、20/21、22/23、24/25`。code0 `0x735b..0x73e2` 确认四个 selector 立即翻转对应字节并重开同页：索引 0 切换 `DS:0x1e61` 并调整 sequence volume；索引 1 切换 `DS:0x1e62`，而 SFX wrappers `0x15a96/0x15b45` 以该字节为播放门；索引 2/3 分别切换 `DS:0x3af9/0x1aab`。后三项标签仍待人工确认。 |
| 0x173e7 | Ghidra 未识别边界 | field_command_first_enabled_select | 从四项 disabled 表选择第一项可用命令 | 当前权威 code0 `0x73e7`；将全局选择索引置 0，再递增到首个值为 0 的项目。三个 object 2 disabled 模板 `DS:0x3ef2/0x3f02/0x3f22` 均由 loader 映射为四个零 dword；调用者复制到栈后再修改动态禁用项。 |
| 0x1741c | Ghidra 未识别边界 | field_command_menu_open | 展开通用战场四向图形菜单 | 当前权威 code0 `0x741c`；播放 FDOTHER[31] SFX 8，并按调用者提供的 command ID／disabled 表从当前焦点四相位展开。单位 action、空焦点、次级菜单和设置页复用该 primitive。 |
| 0x176b4 | Ghidra 未识别边界 | field_command_menu_close | 收起通用战场四向图形菜单 | 当前权威 code0 `0x76b4`；同样播放 SFX 8，按独立初值收起，不是打开坐标的简单倒序。 |
| 0x177fc | Ghidra 未识别边界 | field_command_menu_input | 处理通用四向图形菜单的选择、确认和取消 | 当前权威 code0 `0x77fc`；扫描码 `0x48/0x4b/0x4d/0x50` 选择索引 `0/1/2/3`，disabled 项不改变选择；`0x39/0x1c` 返回 1，`0x01` 返回 `-1`。 |
| 0x17898 | Ghidra 未识别边界 | field_command_menu_wait_key | 等待菜单按键并驱动高亮闪烁 | 当前权威 code0 `0x7898`；等待期间推进战场动画，将 keypad 扫描码 `0x52` 规范化为确认 `0x1c`，将 `0x53` 规范化为取消 `0x01`。 |
| 0x179d5 | Ghidra 未识别边界 | field_command_menu_draw | 按 command ID、disabled 状态和选中高亮绘制四个图标 | 当前权威 code0 `0x79d5`；计算 `frame=command_id*3+disabled*2`，仅对当前选择再加 0/1 高亮相位；帧从 FDOTHER[2] 78 项 offset 表读取。 |
| 0x19df7 | Ghidra 未识别边界 | field_secondary_menu_execute | 显示四项次级 field-level 菜单并分派屏幕页、存档、读档和离开战场 | 当前权威 code0 `0x9df7`。object 2 `DS:0x1ef5={12,13,14,15}`；selector 0 调用 code0 `0xb1e7`（dual `0x1b1e7`）的全屏部队状态总览，按 12 相位展开／收起并逐行显示 actor 属性、stage、turn、资源值和三阵营计数；可见页名仍待人工确认。selector 1 使用 FDTXT `0x19a/0x19b/0x19c` 执行快照写回；selector 2 使用 `0x19d/0x19e` 并调用 `0x15977(-1,0)`；selector 3 在 code0 `0xa25b..0xa306` 使用 `0x19f/0x1a0`，调用 `0x15977(-1,1)` 后返回 `-1`。SDL 将该返回值建模为独立「返回标题」结果，不与宿主关闭合并。 |
| 0x29300 | Ghidra 未识别边界 | field_manual_slot_dispatch | 分派全屏部队状态页、四槽手工保存、四槽手工读取和离开战场确认 | 当前权威 code0 `0x19300` 的干净函数入口；其 selector `0..3` 分别调用 code0 `0x19620`（部队／单位选择页）、`0x1968d`（手工 Save）、`0x1986f`（手工 Load）和 FDTXT `0x19f/0x1a0` 的离开确认。四槽路径使用 `0x312b + selected * 0xa28`，不能解释为标题菜单新游戏分派。 |
| 0x29620 | Ghidra 未识别边界 | field_manual_slot_save | 选择并写入一个四槽手工存档 | code0 `0x1968d`：调用 `0x19bcb(buffer, initial_slot)`，空槽以 `slot+0xa00 == 0xff` 显示；确认后完整复制当前单位表 `0xa00` 字节、写 meta `+0xa00..+0xa09`，重算 checksum、XOR 加密后写回；成功片段为 FDTXT `0x294`。 |
| 0x2986f | Ghidra 未识别边界 | field_manual_slot_load | 选择并读取一个四槽手工存档 | code0 `0x1986f`：调用 `0x19bcb(buffer, initial_slot=1)`，只以 `slot+0xa00 == 0xff` 判定空槽；成功后恢复 `0xa00` 单位表、stage、unit_count、资源值和四个配置字节，并重建运行期单位 cache，随后写 `DS:0x3f56 = 0x238d + stage*0x1f`；成功片段为 FDTXT `0x1de`，不兼容当前入口的 stage 显示 `0x1df`。 |
| 0x29bcb | Ghidra 未识别边界 | field_manual_slot_picker | 绘制并选择四个手工 slot | code0 `0x19bcb(buffer, initial_slot)`：使用 `0x19ab2` 按 `y=0x77 + index*0x13` 逐行绘制四项，扫描码 `0x48/0x50` 将选择限制在 `0..3`，Enter/Space 确认，Esc 取消；每项以 FDTXT `0x225` 开头，空槽绘制 `0x202`，有效槽绘制 `0x202+stage`。该 picker 与 `0x17d33` 的六项滚动选择器无关。 |
...
| 0x38c58 | FUN_0001118c | field_cell_event_lookup | 按地图格事件 ID 与 match 参数查询 FDFIELD metadata 的 cell_lookup 表，并设置 `DAT_00001a8f` | 权威 code0 读取 32 位 cell 的 byte 2 低 5 位作为 1-based ID，检查地形 attr.flags `0x60` 后访问 metadata offset `0x33 + (id-1)*2`。玩家路径每步传 match `0`；玩家行动结束与 `field_ai_unit_execute` common tail 传 `1`。AI 只在最终格执行一次 lookup，不沿移动路径触发 match `0`。 |
| 0x38cb3 | Ghidra 未识别入口 | field_ai_unit_execute | 按单位 AI behavior 执行一名 side 0/1 actor | 正确 dual `0x13a9f`；先拒绝 flags `&0x05`（hidden bit 0 或 AI-ineligible bit 2），再取 record `+0x34 &0x0f` 分派 behavior `0..5、7..11` 并读取 `+0x35/+0x36/+0x3d`。behavior 8 在 `0x13d97..0x13d9a` 直接跳到 function epilogue：不执行 cell lookup、acted 或全表 direction reset；outer side loop 仍调用 stage handler并继续下一个 raw actor。其余正常尾部查询 cell event 并标记 acted。 |
| 0x38cbd | Ghidra 未识别边界 | field_ai_unit_execute_core | `field_ai_unit_execute` 的 `__chkstk` 后主体 | mode 0 为 try-attack → hostile witness → nearest；1 为 try-attack → witness；3/9 以 `+0x35` 查 text ID；4/7/10 以 `+0x35/+0x36` 为坐标；11 为 magic → physical → witness → recovery。behavior 7 到达 `+0x35/+0x36` 后调用正确 dual `0x32975`。mode 2 在 try-attack 失败后只重算 physical candidate，再执行恢复/common tail；mode 5 按 actor `+0x3d` 查找 cell action。common tail 的固定顺序为 `field_cell_event_lookup(x,y,1)` → `field_actor_mark_acted(actor)` → `field_all_actor_directions_reset()` → presentation flush。 |
...
| 0x3a269 | Ghidra 未识别边界 | field_ai_item_action_execute | 执行已选 AI 物品动作的范围、演出和效果提交 | 正确 dual `0x15055`，最终调用正确 dual `0x20c6f(actor,slot,...)`。dispatcher 的 code 5/13 共用 `0x211a4`，逐目标把 item `u16 +0x0e` 传给恢复 core `0x1c916`；每目标消费一次 spread RNG，满 HP 仍消费。只有 code 5 在 `0x20d24` 调用 `field_unit_inventory_slot_remove @0x1b8e7`：左移后续双字节槽并只写末槽 flag `+0x18=0x80`，保留 `+0x19` stale item ID；code 13 保留原 slot。code 20/24 共用 `0x20f6d` wrapper，code 21 调用 `0x2111a`，三者把 value 作为 magic ID 传入 `0x1c75e` 且不移除。 |
| 0x3a525 | Ghidra 未识别边界 | field_ai_magic_action_execute | 执行已选 AI 法术／技能动作的范围、handler 与死亡清理 | 正确 dual `0x15311` 先要求 `DS:0x3c23>=6`；目标列表由 record `+0x04` 范围和 `+0x06` 阵营 filter 构造。IDs `<10` 且全局 `DS:0x3af9==0` 时调用原 `0x55115`／corrected dual `0x2ff01` 战斗动画，否则直接调用 `DS:0x1d01[magic_id]` effect handler；静态复核未发现 actor `+0x44` 扣除，record `+0x05` 仅作为 MP 可用性门。IDs 17/18/19 的 cores `0x22721/0x22866/0x22997` 只在 `+0x22/+0x23/+0x24` 为零时消费一次 duration RNG；随后分别对 attack/defense 加 `round-nearest-even(old*0.15+1)`，或对 accuracy/evasion 做 16 位 `+15`。IDs 20/21 经 wrappers `0x22a85/0x22bc6` 把 `+0x25/+0x26` 传给共享 `0x22af6`，仅清除非零 byte且不消费 RNG。ID 25 的 `0x22c04` 仅在 acted bit 已置位时执行 `flags &= 0x7f`，保留其他 flag bits且不消费 RNG。IDs 22/26/27 共用 `0x22cda` 的状态为零、profile 25/26 免疫、50% gate、固定基础伤害 10、duration RNG 顺序；behavior 11 最后才执行一次 common tail。 |
| 0x3f51f | FUN_00017a53 | field_turn_cycle_run | 玩家阶段结束后依次处理 side 1、side 0，递增回合后进入 side 2 | 反编译调用顺序为 `field_turn_event_check(1)`、side 1 phase 前置状态处理与 `field_side1_phase_execute`、清行动标志、`field_turn_event_check(0)`、side 0 phase 前置状态处理与 `field_side0_phase_execute`、`DAT_00003bef++`、清行动标志、`field_turn_event_check(2)`；机器码 `0x3f716..0x3f731` 比较 primary/alternate 并按需淡出，`0x3f78e` 在 phase 0 事件后读取 `DS:0x1e81[stage]` 启动 alternate，`0x3f824` 在新回合 phase 2 前恢复 `DS:0x1e63[stage]` primary。SDL 已复现可观察的中毒扣 `maxHP/10`、`+0x22..+0x27` 状态递减，以及完整 actor 表 acted 清理时机。|
...
| 0x4e866 | FUN_00073a7a | field_magic_record_get | 返回 7 字节法术／技能静态记录 | `DS:0x19fd + magic_id*7`；36 条记录来自 object3 `DS:0x19fd` / `FD2.EXE` file offset `0x7aa11`。record `+0x06` 已由 AI 候选与执行层交叉确认为目标阵营 filter：selector mode 0 对其零／非零做反转，非零 selector mode 直接使用该值。 |
| 0x55115 | FUN_00055115 | field_magic_battle_animation | IDs 0..9 的战斗法术动画 wrapper | 原地址 `0x55115` 是 `__chkstk(0x164)` 入口；corrected dual 主体 `0x2ff01`。它按 magic ID 选择战斗动画与分镜，并在逐目标动画帧通过 `DS:0x24c6[magic_id]` effect 表提交结果；不是独立于 `field_magic_damage_profile_apply` 的第二套 gameplay 公式。 |
| 0x5511f | FUN_0005511f | field_magic_battle_animation_core | `field_magic_battle_animation` 的 `__chkstk` 后主体 | corrected dual `0x2ff01`；ID `>=32` 转专用流程，ID 24／28..31 转其他专用动画，常规分支覆盖 IDs 0..27。 |
| 0x1c768 | merged into object1 code | field_magic_damage_profile_apply | 复制 28 项 magic profile scale 并按 actor profile 计算法术基础值 | object1 code0 `0xc768` 的 `mov esi,0x1f96; rep movsd` 由 fixup 重定位到 object2 `0x51f96`；倍率为 `{10,10,10,10,7,7,10,10,10,10,9,10,5,5,8,10,6,8,10,9,5,5,10,8,8,4,10,7}`。IDs 0..8 的 `DS:0x1d01` wrappers 进入 `field_magic_battle_animation`，后者在 corrected dual `0x30958` 逐目标调用该 core；ID 9 wrapper @`0x214ad` 直接调用它。只有 IDs 10..12 在命中 RNG 前额外调用 `field_unit_ignores_terrain_combat_modifier @0x44397` 免疫门。命中使用 magic `+2`，伤害再消费一次 RNG 并按 `90%..99.9%` 浮动。 |
| 0x1c8ed | merged into object1 code | field_magic_heal_apply | 读取 magic record 的恢复量并调用共享 HP 恢复 core | DS:0x1d01 的 IDs 13..16 wrappers `0x21ad9/0x21b99/0x2211c/0x22153` 最终逐目标调用该入口；它通过 `field_magic_record_get` 读取 `u16 +0` 后转入 corrected dual `0x1c916`。物品 code 5/13 则由 `0x211a4` 直接把 item value 传给同一 core。无命中判定或 profile 免疫；每目标固定消费一次 spread RNG，按 `power*9/10 + (rng%100)*power/1000` 两段截断后加 HP并 cap 到 maxHP；满 HP 仍消费 RNG。 |
...
| 0x4096e | legacy FUN_00018e98 body | field_unit_combat_stats_recompute | 从 record 基础值和 8 个装备槽重算 attack、defense、accuracy、evasion | corrected code0 `0x3096e` 读取 `0x37/0x39/0x3e` 与 `0x0a..0x19`；DOSBox CPU trace 确认共享尾部执行 `mov [edi+0x4e],ax`，与静态写入 `0x48/0x4a/0x4c` 合并为四项派生值；状态归零路径复用同一派生链。|
...
| 0x42a1f | Ghidra 未识别入口 | field_side1_phase_execute | 顺序执行 side 1 单位阶段 | corrected code0 `0x32a1f` 遍历 actor 表，每个 actor 前将 `DS:0x1a8f` 置 `0xff`；仅对 side 1、flags `&0x81==0`、record `+0x26==0` 的单位调用 `field_ai_unit_execute(actor,1)`，AI 入口再拒绝 flags bit `0x04`。AI 返回后若 `DS:0x1a8f!=0xff`，立即以事件码索引 `DS:0x1b91` 并传 actor index；随后才执行 stage handler。 |
| 0x42a29 | Ghidra 未识别边界 | field_side1_phase_execute_core | `field_side1_phase_execute` 的 `__chkstk` 后主体 | actor 顺序严格为索引递增；`DS:0x1a8f` 在 AI gate 前复位，且无论 actor 的 side、flags、状态或 AI 入口是否允许执行，outer loop 都会先处理可能的 cell handler，再无条件调用 `DS:0x1b19[stage](actor_index)`，最后检查 `DS:0x3ecc`。该标志属于剧情／脚本退出控制，不是通用 battle result。 |
| 0x42ace | Ghidra 未识别入口 | field_side0_phase_execute | 两阶段执行 side 0 单位阶段 | 正确 dual `0x1d8ba`：首轮按 raw actor index 计算 magic/item 候选，任一 score `>=6` 才执行 AI；两轮均由 AI 入口拒绝 flags bit `0x04`，第二轮从索引 0 重新扫描尚未 acted 的可行动单位。SDL 以 `phase_ai_pass/phase_unit_cursor` 保持两轮各自的稳定顺序。 |
| 0x1a866 | Ghidra 未识别边界 | field_phase_status_tick | phase 前置状态处理 | corrected dual `0x1a866` 的 poison 与状态子循环均从 raw actor index 0 开始，仅处理匹配 side 且 flags bit 0 未置位的 actor；poison 扣除 `HP -= maxHP/10` 并下限为 0，随后遍历 `record +0x22..+0x27` 六个状态 byte，每次归零都会调用 `field_unit_combat_stats_recompute_runtime`，最终派生属性复用 `0x4096e`，倍率为 DS:0x018d 的 1.15。 |
| 0x1b750 | Ghidra 未识别边界 | field_unit_combat_stats_recompute_runtime | phase 状态归零时的派生属性重算 | corrected dual `0x1b750` 读取同一单位记录和装备槽，复用 `0x4096e` 的基础/装备累加与 16 位写回；AP/DP 状态有效时对对应派生值执行 1.15 x87 nearest-even 缩放。 |
| 0x42ad8 | Ghidra 未识别边界 | field_side0_phase_execute_core | `field_side0_phase_execute` 的 `__chkstk` 后主体 | 两轮均在每个 raw actor 前将 `DS:0x1a8f` 置 `0xff`；无论该 actor 是否实际进入 AI，之后都先分派可能的 `DS:0x1b91[event_code](actor_index)`，再无条件调用 `DS:0x1b19[stage](actor_index)` 并检查 `DS:0x3ecc`。`0x205be` 会将退出标志置 2，普通退出处理可置 1；该标志不是通用 battle result。`DS:0x1b19[0]` 的 stage 0 handler 正确 dual 为 `0x205b4`。 |
...
| 0x4df09 | Ghidra 未识别边界 | save_checksum_sum | 对 FD2.SAV 除末尾 4 字节外的所有明文字节求 32 位和 | 当前权威 code0 `0x3df09`，dual `0x4df09`；循环长度为 `size-4`。本地原版存档解密后 `sum(data[0:-4]) == trailing_u32 == 0x004527c1`。旧 Ghidra `FUN_0007311d` 是不同高地址函数，不再混用。 |
| 0x4df28 | Ghidra 未识别边界 | save_xor_crypt | FD2.SAV 对称 XOR 加/解密流 | 当前权威 code0 `0x3df28`，dual `0x4df28`；`state=0x00a5`，每字节 `rol16(state+0x9014,3)` 后 XOR。旧 Ghidra `FUN_0004b670 @0x7313c` 映射已废弃。 |
