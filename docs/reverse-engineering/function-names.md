...
| 0x3a269 | Ghidra 未识别边界 | field_ai_item_action_execute | 执行已选 AI 物品动作的范围、演出和效果提交 | corrected code0 `0x2a269` 使用 `DS:0x3c37/3b/3f`，最终调用 `0x45e83(actor,slot,...)` 后标记 acted；`0x45e83` 内的效果／消耗尚未完成逆向。 |
| 0x3a525 | Ghidra 未识别边界 | field_ai_magic_action_execute | 执行已选 AI 法术／技能动作的范围、handler 与死亡清理 | corrected code0 `0x2a525` 使用 `DS:0x3c23/27/2b/2f`，分派 `0x55115` 或 `DS:0x1d01` handler 表；当前静态／clean code0 复核未发现 `record+0x44` 扣除，`+0x05` 仅作为可用性门；AI core common tail 在 handler 返回后负责 cell event、acted 和 transient cleanup。SDL 对未确认 handler 保持显式拒绝。 |
...
| 0x1c768 | merged into object1 code | field_magic_damage_profile_apply | 复制 28 项 magic profile scale 并按 actor profile 计算法术基础值 | object1 code0 `0xc768` 的 `mov esi,0x1f96; rep movsd` 由 fixup 重定位到 object2 `0x51f96`；28 项静态表位于 file `0x76faa`。 |
