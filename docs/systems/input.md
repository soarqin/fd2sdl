...
页面等待不只出现在 `-3` 和 fragment 结尾。`text_dialog_render_tokens` 处理 `-17..-20` 说话人控制码时，若旧对话框已打开，会先调用 `FUN_00016c57(0)` 等待并读取一次按键，再关闭旧框并打开新说话人的框。因此同一 fragment 内每次说话人切换都是独立等待边界；不能直接关闭旧框后继续渲染下一句。
